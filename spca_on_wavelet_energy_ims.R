...
dat = read.csv("D:/rfiles/feature_matrix_wav_energy8_1024_ims.csv",header = T)
...
write.csv(round(regular.pca$rotation[,1:2],3),'loadings_pca_wav_energy8_ims.csv',
          row.names = F)
...
ggsave("pecentage_var_expl_wav_energy8_ims.jpg",width = 4,height = 4)
...
ggsave("regular_pca_projection_wav_energy8_ims.jpg",width = 4,height = 4)
...
write.csv(round(spca.result_4$loadings,3),'loadings_spca_wav_energy8_ims_4.csv',
          row.names = F)
spca.result_4
...
ggplot(ggdata, 
  ...
  geom_point()+labs(x = 'Modified PC score along PC1',
                    ...
                    col = 'Fault Type')+
  scale_color_manual(values = c('red','blue','purple','green'))
ggsave("spca_projection_wav_energy8_ims.jpg",width = 4,height = 4)
...
########################
# Using SPCA features (wavelet energy)
train.sub = subset(train,select= c(paste0("E",c(1,6:8)),'fault_type'))
test.sub = subset(test,select = c(paste0("E",c(1,6:8)),'fault_type'))
...
train.sub = subset(train,select= c(paste0("E",c(5:8)),'fault_type'))
test.sub = subset(test,select = c(paste0("E",c(5:8)),'fault_type'))
svmfit = svm(fault_type~.,data = train.sub,kernel = 'radial',
             cost = 1,gamma = 0.05)
table(svmfit$fitted,train.sub[,dim(train.sub)[2]])
pred.test.sub = predict(svmfit,test.sub)
...
ggsave("random_training_wav_energy8_ims_4.jpg",width = 4, height = 4)
